...
def list_simulations(db: Session, user_id: UUID) -> list[Simulation]:
    return (
        db.query(Simulation)
        .filter(Simulation.user_id == str(user_id))
...
def get_simulation(db: Session, user_id: UUID, sim_id: UUID) -> Simulation | None:
    return (
        db.query(Simulation)
        .filter(Simulation.id == str(sim_id), Simulation.user_id == str(user_id))
        .first()
    )
...
def save_simulation(db, user_id, payload, results) -> Simulation:
    sim = Simulation(
        user_id=str(user_id),
